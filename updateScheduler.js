...
const { UPDATE_WINDOW_START_HOUR, UPDATE_WINDOW_END_HOUR } = require("./config");
...
function startUpdateScheduler({ state, saveState, publicManager, privateManager, publicRoot, privateRoot, notify }) {
    async function checkAndUpdate() {
        ...
        const targets = [
            { root: publicRoot, manager: publicManager },
            { root: privateRoot, manager: privateManager }
        ];

        for (const { root, manager } of targets) {
            try {
                ...
                if (result.updated) {
                    console.log(` Server ${manager.label} updated: ${result.from || "?"} -> ${result.to}`);
                    if (notify) await notify(` Server ${manager.label} updated to version ${result.to}.`);
                } else {
